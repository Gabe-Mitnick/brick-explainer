...
export default function TextLayer({ moments, currentMoment }: Props) {
	...
	const [panels, setPanels] = useState<Panel[]>([{ stepMoment: 0, substepMoments: [], animClass: null, key: 0 }])
	...
	useEffect(() => {
		...
		if (currSlideStart !== prevSlideStart) {
			...
			setPanels((prev) => {
				const updated = prev.map((p) => (p.animClass?.startsWith('exit') ? p : { ...p, animClass: exitAnim }))
				return [
					...updated,
					{
			...
			setPanels((prev) =>
				prev.map((p) => (p.animClass?.startsWith('exit') ? p : { ...p, substepMoments: newSubstepMoments })),
			)
		}
